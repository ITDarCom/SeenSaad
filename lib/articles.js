...
//addUpdateSchema = $.extend({}, ArticleSchema,articleTextSchema);
...
articleTexts = new Mongo.Collection('articleTexts');
articleTextSchema = new SimpleSchema({
    articleId: {
        type: String,
        autoform: {omit: true},
        custom: function () {
    ...
    text: {
        ...
        autoValue: function () {
            if (Meteor.isServer) {
                ...
                if (this.isUpdate) {
                    if (((new Date()).getTime() - articleTexts.findOne(this.docId).createdAt) > 600 * 1000) {
                        this.unset();
                    }
                }
            }
...
articleTexts.attachSchema(articleTextSchema);
articleTexts.allow({
...
addUpdateSchema = new SimpleSchema([ArticleSchema, articleTextSchema]);
