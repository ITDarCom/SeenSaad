...
Articles.attachSchema(new SimpleSchema({  // simpleSchema from package collection2 used to organize the operation of (insert-update) opretions in collections
    ...
    readingPermissions: {
        ...
        autoform: {
            options: function () {
                var contributingIds = AutoForm.getFieldValue('contributingIds');
                return _.filter(Meteor.users.find({_id: {$not: Meteor.userId()}}).map(function (c) {
                    return {label: c.username, value: c._id};
                    // //return users name for select2 field
                }), function (u) {
                    if (!_.contains(contributingIds, u.value))
                        return u
                });
            },
            type: function () {
