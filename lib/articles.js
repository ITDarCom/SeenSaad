...
// opretions in collections
    ...
    body: {
        type: String,
        min: 3,
        custom: function () {//To validate specific key (body) key in a specific (ArticleSchema) schema
            var regex = /(<([^>]+)>)/ig;
            ...
            if (this.value.replace(regex, "").replace(/&nbsp;/g, '').length < 3) {
                    return 'minString'
            }
        },
        label: 'الموضوع',
    ...
    read: {  //this key name must to be readcount
        type: Number,
        autoform: {omit: true},
        autoValue: function () {
            if (this.isInsert) {
    ...
    readingIds: { //this field must to be reading name.
        type: [String],
...
commentsSchema = (new SimpleSchema({
    articleId: {
        ...
        custom: function () {
            ...
            if (!article || (article.contributingPermissions != 0 && !_.contains(article.contributingIds, Meteor.userId()))) {
