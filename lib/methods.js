...
Meteor.methods({
    ...
    permissionUpdate: function (articleId, readingIds, contributingIds) {
        check(articleId, String);
        check(newContributingIds, [String]);
        check(newReadingIds, [String]);
        var doc = Articles.findOne(articleId);
        if (doc && doc.user == this.userId) {
            if (doc.contributingPermissions) {
            ...
            var newContributingIds = _.difference(doc.contributingIds, contributingIds);
            var deletedContributingIds = _.difference(contributingIds, doc.contributingIds);
            var newReadingIds = _.difference(doc.readingIds, readingIds);
            var deletedReadingIds = _.difference(readingIds, doc.readingIds);
