...
Meteor.methods({
    ...
    addExtension: function (doc) {
        ...
        if (article && article.user == this.userId) {
            if (!allowedUpdateTime(article.createdAt)) {
                ...
                if (article && article.user == this.userId) {
                    if (!allowedUpdateTime(articleTexts.findOne({articleId: doc.articleId}, {sort: {createdAt: -1}}).createdAt)) {
                        articleTexts.insert(doc);
                    }
    ...
    addArticle: function (doc) {
        if (Meteor.userId()) {
            addUpdateSchema.clean(doc);

            var result = Articles.insert(_.omit(doc, 'articleId', 'text'), function (err, result) {
                if (!err) {
                    Meteor.call('permissionDeploy', result);
                }
            });
            articleTexts.insert({text: doc.text, articleId: result, createdAtText: new Date()});
            return result;
        }
    }
});
