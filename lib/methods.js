...
Meteor.methods({
    ...
    seenChange: function (articleId) {
        ...
        }, {$set: {'readingArticles.$.seen': true, 'readingArticles.$.newComment': true}});
        Stream.update({
            userId: Meteor.userId(),
            'contributingArticles.id': articleId
        }, {$set: {'contributingArticles.$.seen': true, 'contributingArticles.$.newComment': true}});
    }
    ,
                    ...
                    if (!err) {
                        ...
                        if (article.contributingPermissions == 1 || article.readingPermissions == 1) {
                            _.each(article.contributingIds, function (userId) {
                                Stream.update({
                                    userId: userId,
                                    'contributingIds.id': articleId
                                }, {$set: {newComment: true}});
                            });
                            _.each(article.readingIds, function (userId) {
                                Stream.update({userId: userId, 'readingIds.id': articleId}, {$set: {newComment: true}});
                            });
                        }
                    }
                })
