...
Meteor.methods({
    removeArticle: function (docId) {
        ...
        if (article && article.user == this.userId)
            Articles.remove(docId, function (err, result) {
                if (!err) {
                    if (Meteor.isClient) {
                        Router.go('mine');
                    }
        ...
        if (doc && doc.user == this.userId) {
            ...
            if (doc.readingIds != null) {
                _.each(doc.readingIds, function (id) {
                    ...
                    else// object that contain the keys numberAffected and insertedId
    ...
    },//as long as you use the name :"edit" with routing when you want to do update, why did not use same name with update?
...
allowedUpdateTime = function (date, flag) {//if flag is true the date is converted by getTime() else the date is iso date type so we should convert it
    var converted;
    if (!flag) {
...
Addition = {
    date: function (addition) {//adition parameter is the text of an adition
