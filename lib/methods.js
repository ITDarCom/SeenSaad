...
Meteor.methods({
    ...
    updateProfile: function (user) {
        ...
        //Meteor.users.update({_id: Meteor.userId()}, {$set: {email: user.email.address,mobile:user.mobile.number
        //    ,fullName:user.fullName,birhtday:user.birthday,gender:user.gender,mobilePrivacy:user.mobilePrivacy
        //    ,fullNamePrivacy:user.fullNamePrivacy,genderPrivacy:user.genderPrivacy,birthdayPrivacy:user.birthdayPrivacy
...
emailSchema = new SimpleSchema({
    address: {
        ...
        custom: function () {
            ...
            if (user.email) {
                if (user.email.address != this.value && !Meteor.users.findOne({'email.address': this.value})) {
                    return "emailUsed"
                }
            }
            else if (!Meteor.users.findOne({'email.address': this.value})) {
                return "emailUsed"
            }
