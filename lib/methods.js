...
Meteor.methods({
    ...
    seenChangeMsg: function (id) {
        ...
        Chatlist.update({userId: msg.to, participant: msg.from}, {$set: {seen: true}})
    },
    addComment: function (doc) {
        if (Meteor.userId()) {
            doc.commenter = Meteor.userId();
            doc.commenterUsername = Meteor.user().username;
                        ...
                        if (!err) {
                            ...
                            if (article.contributingPermissions == 1 || article.readingPermissions == 1) {
                                ...
                                _.each(article.readingIds, function (userId) {
                                    Stream.update({
                                        userId: userId,
                                        'readingArticles.id': doc.articleId
                                    }, {$set: {'readingArticles.$.newComment': true}});
                                });
                            }
                        }
                    })
    ...
    ,
    getMsgCount: function (id) {

        return Messages.find({$or: [{from: id}, {to: id}]}).count()
    ...
    removeArticle: function (docId) {
        ...
        if (article && article.user == this.userId) {
            Meteor.call("removePermissions", article._id, function (err, res) {

                Articles.update({_id: docId}, {$set: {deleted: true}}, {filter: false});
            })
            ;
        }
    },
    ...
    setLoginTime: function () {
        Meteor.users.update(Meteor.userId(), {$set: {lastSignIn: new Date()}})
    },
    getRegisteredAt: function (id) {
        var user = Meteor.users.findOne(id);
    ...
    sendMsg: function (doc) {
        if (this.userId) {
            ...
            doc = messagesSchema.clean(doc);
            check(doc, messagesSchema);

            Messages.insert(doc, function (result) {
                Chatlist.upsert({userId: Meteor.userId(), participant: doc.to}, {
                    $set: {
                ...
                Chatlist.upsert({userId: doc.to, participant: Meteor.userId()}, {
                    $set: {
                        lastMessage: doc.message,
                        sentAt: doc.sendingAt,
                        direction: false,
