...
personalInformationSchema = new SimpleSchema({
    username: {
        ...
        custom: function () {
            ...
            if (Meteor.users.findOne({$and:[{username: regex},{_id:{$ne:Meteor.userId()}}]}))
                return "usedUserName";
            //noinspection JSUnresolvedVariable
            if (this.value.indexOf("--") != -1 || this.value.length < 4 || this.value.length > 16  )
...
AccountsTemplates.addFields([
    {
        ...
        required: true,
        func: function (value) {
            if ((!usernameRegex.test(value) && value.indexOf("--") != -1) || value < 4 || value > 30 || _.contains(value, " ")) {
                return true;
            }
            return false
        },
    ...
    if (!error) {
        ...
        if (state == 'signUp') {
            ...
            Counts.insert({userId:Meteor.userId(),count:0})
        }
    }
