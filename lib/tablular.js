...
TabularTables.users = new Tabular.Table({
    ...
    columns: [
        ...
        {data: "messageCounter", title: '<i class="fa fa-envelope" aria-hidden="true"></i>  ',
            tmpl: Meteor.isClient && Template.messagesCounterTemplate,
            "searchable": false},
        {
        ...
        ,
        {
            data: "getRegisteredAt()", title: arabicMessages.registeredAt,
            tmpl: Meteor.isClient && Template.registeredAtTemplate,
            "searchable": false
        }
...
Meteor.users.helpers({
    ...
    messageCounter: function () {
        ...
        var id = this._id;
        Meteor.call('getMsgCount',this._id,function(err,result){
            if (!err) {
                var htmlId = '#msgCount' + id;
                $(htmlId).text(result);
    ...
    getRegisteredAt: function () {
        var id = this._id;
        Meteor.call('getRegisteredAt', this._id, function (err, result) {
            if (!err) {
                var htmlId = '#registeredAt' + id;
                $(htmlId).text(result);
            }
        })
    }
